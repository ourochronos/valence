...
def sybil_cluster_graph(node_ids):
    ...
    return {
        ...
        node_ids["carol"]: {node_ids["alice"]: 0.95, node_ids["bob"]: 0.95, node_ids["dave"]: 0.3},
...
class TestRingDetector:
    ...
    def test_ring_coefficient_decreases_with_size(self, node_ids):
        ...
        # 5-node ring
        path_5 = [
            node_ids["alice"], node_ids["bob"], node_ids["carol"],
            node_ids["dave"],
        ]
...
class TestSybilClusterDetector:
    ...
    def test_detect_dense_cluster(self, sybil_cluster_graph, node_ids):
        ...
        sybil_cluster = next(
            (c for c in clusters if set(c.node_ids) & sybil_nodes),
            None
        )
    ...
    def test_temporal_correlation_detection(self, node_ids):
        ...
        activity_times = {
            ...
            node_ids["bob"]: [base_time + timedelta(minutes=5), base_time + timedelta(hours=1, minutes=10)],
            node_ids["carol"]: [base_time + timedelta(minutes=10), base_time + timedelta(hours=1, minutes=5)],
        }
...
class TestIntegrationWithTrustPropagation:
    ...
    def test_ring_coefficient_applied_to_propagation(self, ring_graph, node_ids):
        ...
        result_without = engine_without_ring.compute_transitive_trust(
            node_ids["alice"], node_ids["carol"]
        )
