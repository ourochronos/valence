...
class TestNodeCircuits:
    ...
    def test_node_circuit_config(self, node_client):
        """Test node circuit configuration."""
        assert node_client.circuit_min_hops == 2
        assert node_client.circuit_max_hops == 3
        assert node_client.circuit_lifetime == 600.0
        assert node_client.use_circuits is True

    def test_node_get_router_subnet(self, node_client):
        """Test router subnet extraction."""
        router = RouterInfo(
            router_id="a" * 64,
        ...
        subnet = node_client._get_router_subnet(router)
        assert subnet == "10.0.0.0/16"

    def test_node_get_router_subnet_ipv6(self, node_client):
        """Test router subnet extraction for IPv6."""
        router = RouterInfo(
            router_id="a" * 64,
        ...
        subnet = node_client._get_router_subnet(router)
        assert "2001:db8::" in subnet

    def test_node_select_diverse_routers(self, node_client, mock_router_info):
        """Test diverse router selection."""
        routers = [
            mock_router_info("a" * 64, "10.0.1.1:8471"),
            mock_router_info("b" * 64, "10.0.2.1:8471"),  # Same /16
            mock_router_info("c" * 64, "10.1.1.1:8471"),  # Different /16
            mock_router_info("d" * 64, "10.2.1.1:8471"),  # Different /16
        ]

        selected = node_client._select_diverse_routers(routers, 3)

        # Should prefer diverse subnets
        assert len(selected) == 3

        # Check that we got routers from different subnets
        subnets = set()
        for r in selected:
            subnet = node_client._get_router_subnet(r)
            subnets.add(subnet)

        assert len(subnets) >= 2  # Should have at least 2 different subnets

    def test_node_get_circuit_stats(self, node_client):
        """Test node circuit statistics."""
        stats = node_client.get_circuit_stats()

        assert "circuits_active" in stats
        assert "circuits_built" in stats
        assert "messages_via_circuit" in stats

    @pytest.mark.asyncio
    async def test_node_destroy_circuit(self, node_client):
        """Test circuit destruction."""
        # Create a mock circuit
        circuit = Circuit(
            circuit_id="test-circuit",
            hops=[CircuitHop(router_id="r" * 64)],
        )
        node_client._circuits[circuit.circuit_id] = circuit
        node_client._circuit_keys[circuit.circuit_id] = [b"x" * 32]
        node_client._active_circuit_id = circuit.circuit_id

        # Destroy it
        await node_client._destroy_circuit(circuit.circuit_id, "test")

        assert circuit.circuit_id not in node_client._circuits
        assert node_client._active_circuit_id is None
