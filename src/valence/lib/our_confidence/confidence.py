...
def _compute_overall(
    ...
    Per spec (MATH.md):
    - Geometric mean: (prod v_i^{w_i})^{1/sum w_i} - penalizes imbalanced vectors
    - Arithmetic mean: sum(w_i * v_i) / sum w_i - traditional weighted average
    ...
    if use_geometric:
        # Weighted geometric mean: (prod v_i^{w_i})^{1/sum w_i}
        # Use log-space for numerical stability: exp(sum(w_i * log(v_i)) / sum w_i)
        log_sum = 0.0
        total_weight = 0.0
        ...
        # Weighted arithmetic mean: sum(w_i * v_i) / sum w_i
        weighted_sum = 0.0
        total_weight = 0.0
...
class DimensionalConfidence:
    ...
    def _validate_against_registry(self) -> None:
        ...
        if not self.dimensions:
            return
        from .dimension_registry import get_registry
...
def aggregate_confidence(
    ...
    if method == "minimum":
        ...
        def weighted_geo(dim: str) -> float | None:
            ...
            if not values:
                return None
            # Geometric mean in log space: exp(sum(w * log(v)) / sum w)
        ...
        log_sum = sum(w * math.log(max(EPSILON, v)) for v, w in zip(overall_values, overall_weights, strict=True))
