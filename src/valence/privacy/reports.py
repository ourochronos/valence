    ...
    def to_csv(self) -> Dict[str, str]:
        ...
        if self.beliefs:
            ...
            writer = csv.DictWriter(
                output,
                fieldnames=["belief_id", "content", "confidence", "domains", 
        ...
        if self.shares_sent:
            ...
            writer = csv.DictWriter(
                output,
                fieldnames=["share_id", "belief_id", "sharer_did", "recipient_did",
                           "created_at", "policy_level", "revoked", "revoked_at"],
            )
            writer.writeheader()
        ...
        if self.trust_outgoing:
            ...
            writer = csv.DictWriter(
                output,
                fieldnames=["source_did", "target_did", "competence", "integrity",
                           "confidentiality", "judgment", "domain", "created_at",
        ...
        if self.audit_events:
            ...
            writer = csv.DictWriter(
                output,
                fieldnames=["event_id", "event_type", "actor_did", "target_did",
                           "resource", "action", "success", "timestamp", "metadata"],
            )
            writer.writeheader()
...
class ReportDataSource(Protocol):
    ...
    async def get_beliefs_for_user(
        self, user_did: str, domains: Optional[List[str]] = None,
    ...
    async def get_shares_sent(
        self, user_did: str, start_date: Optional[datetime] = None,
        end_date: Optional[datetime] = None,
...
class ReportStore(Protocol):
    ...
    async def update_status(
        self, report_id: str, status: ReportStatus,
        error_message: Optional[str] = None,
    ) -> None:
    ...
    async def update_status(
        ...
        if report_id in self._metadata:
            ...
            if status == ReportStatus.COMPLETED:
                self._metadata[report_id].generated_at = datetime.now(timezone.utc)
