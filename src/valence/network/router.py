...
class CircuitHopState:
    ...
    prev_hop: str | None  # Node ID of previous hop (or originator)
    next_hop: str | None  # Node ID of next hop (or None if exit)
    created_at: float
    message_count: int = 0
...
class CircuitState:
    ...
    def get_circuit(self, circuit_id: str) -> CircuitHopState | None:
...
class RouterNode:
    ...
    async def _activate_back_pressure(self, load_pct: float) -> None:
        ...
        logger.warning(f"Back-pressure ACTIVATED at {load_pct:.1f}% load (threshold: {self.back_pressure_threshold}%)")
    ...
    async def _release_back_pressure(self, load_pct: float) -> None:
        ...
        logger.info(f"Back-pressure RELEASED at {load_pct:.1f}% load (threshold: {self.back_pressure_release_threshold}%)")
    ...
    async def _handle_identify(self, data: dict[str, Any], ws: web.WebSocketResponse) -> str | None:
        ...
        if node_id in self.connection_history:
            ...
            logger.info(f"Node reconnected: {node_id[:16]}... (away for {history.time_since_disconnect():.1f}s, queued: {queued_count})")
        else:
    ...
    async def _deliver_queued(self, node_id: str, ws: web.WebSocketResponse) -> int:
        ...
        return web.json_response(
            {
                ...
                "queues": {
                    ...
                    "total_messages": sum(len(q) for q in self.offline_queues.values()),
                },
                "back_pressure": {
                    "active": self._back_pressure_active,
                    "load_pct": round(self.get_load_pct(), 1),
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(registration_url, json=registration) as response:
                    ...
                    if result.get("status") == "accepted":
                        ...
                        logger.info(f"Registered with seed node {self._seed_id}: {url}")
                        return True
                    else:
                        reason = result.get("reason", "unknown")
                        logger.warning(f"Seed registration rejected: {reason}")
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(heartbeat_url, json=heartbeat) as response:
                    if response.status == 200:
                        ...
                        logger.debug(f"Heartbeat acknowledged: status={health_status}, next_in={next_interval}s")
                        return True
                    else:
    ...
    async def _deliver_circuit_payload(
        ...
        try:
            ...
            if recipient_id in self.connections:
                ...
                self.messages_delivered += 1
                logger.debug(f"Delivered circuit message {message_id[:8] if message_id else 'unknown'}... to {recipient_id[:16]}...")
            else:
